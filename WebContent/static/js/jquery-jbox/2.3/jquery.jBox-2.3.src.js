...
(function ($) {
    $.jBox = function (content, options) {
        ...
        if (!options.isTip) {
			...
			$box.find('.jbox-border').removeClass('jbox-border');
        }
        if (data.type != 'IFRAME') {
            $box.find('#jbox-states .jbox-state:first .jbox-button-focus').focus();
        }
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var top = options.title == undefined ? 10 : 54;
    ...
    $.jBox.tip = function (content, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div class="jbox-content-detail '+options.icon+'">');
      	if( options.icon == 'loading' ){
			html.push('<span class="jbox-icon jbox-icon-' + options.icon + '" style="position:absolute;top:' + (17 + options.border) + 'px;left:' + (17 + options.border) + 'px; width:32px; height:32px;"></span>');
		}else{
			html.push('<span style="position:absolute;top:' + (9 + options.border) + 'px;left:' + (9 + options.border) + 'px; font-size:30px;"><i class="'+ico[options.icon]+'"></i></span>');
		}
        html.push(options.content);
        html.push('</div>');
        options.content = html.join('');

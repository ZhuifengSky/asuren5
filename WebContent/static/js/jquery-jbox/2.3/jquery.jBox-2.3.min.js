...
});P.find(".jbox-button-panel button:eq("+O.buttonsFocus+")").addClass("jbox-button-focus")});var v=function(){n.css({top:l.scrollTop()});if(c.isMessager){o.css({position:(d)?"absolute":"fixed",right:1,bottom:1})}};var w=function(){var N=l.width();return document.body.clientWidth<N?N:document.body.clientWidth};var x=function(){var N=l.height();return document.body.clientHeight<N?N:document.body.clientHeight};var y=function(){if(!c.showFade){return}if(c.persistent||1){var N=0;n.addClass("jbox-warning");var O=setInterval(function(){n.toggleClass("jbox-warning");if(N++>1){clearInterval(O);n.removeClass("jbox-warning")}},100)}else{I()}};var z=function(N){if(c.isTip||c.isMessager){return false}var O=(window.event)?event.keyCode:N.keyCode;if(O==27){I()}if(O==9){var P=a(":input:enabled:visible",n);var Q=!N.shiftKey&&N.target==P[P.length-1];var R=N.shiftKey&&N.target==P[0];if(Q||R){setTimeout(function(){if(!P){return}var S=P[R===true?P.length-1:0];if(S){S.focus()}},10);return false}}};var A=function(){if(c.showFade){p.css({position:"absolute",height:c.isTip?x():l.height(),width:d?l.width():"100%",top:0,left:0,right:0,bottom:0})}fade(a("body, iframe"),function(obj,ifr,top,left){ifr.find("#jbox-fade"+(c.showFade?"":"2")).css({position:"absolute",height:o.height(),width:o.width(),top:o.offset().top-top,left:o.offset().left-left,right:0,bottom:0,zIndex:c.zIndex,opacity:0})})};var B=function(){if(c.isMessager){o.css({position:(d)?"absolute":"fixed",right:1,bottom:1})}else{q.css({top:c.top});o.css({position:"absolute",top:q.offset().top+(c.isTip?l.scrollTop():0),left:((l.width()-o.outerWidth())/2)})}if((c.showFade&&!c.isTip)||(!c.showFade&&!c.isTip&&!c.isMessager)){n.css({position:(d)?"absolute":"fixed",height:c.showFade?l.height():0,width:"100%",top:(d)?l.scrollTop():0,left:0,right:0,bottom:0})}A()};var C=function(){c.zIndex=a.jBox.defaults.zIndex++;n.css({zIndex:c.zIndex});o.css({zIndex:c.zIndex+1})};var D=function(){c.zIndex=a.jBox.defaults.zIndex++;n.css({zIndex:c.zIndex});o.css({display:"none",zIndex:c.zIndex+1});if(c.showFade){p.css({display:"none",zIndex:c.zIndex,opacity:c.opacity})
